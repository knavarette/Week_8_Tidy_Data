...
```{r}
new_youth %>% 
  _______(gender, grade, perception_of_weight) %>%  #drop the missing data
  _______(gender, grade, perception_of_weight) %>% #count individuals in these groups
  _______(gender, grade) %>% #group the values together
  _______(total = sum(n),    
         proportion = round(n/total, 4)) %>% #create new variables "total" and "proportion"
  _______(gender, grade, perception_of_weight, proportion) %>% #choose only these columns
  pivot_______(______from = perception_of_weight, 
              ______from = proportion) %>% #make our date wider with column titles as the perception and the values as the proportion
...
```{r}
child %>% 
  pivot_______(cols = ______, names_to = ________, values_to = ______) 
```
...
new_child %>% 
  ...
  __________(year) %>%  #group together values by year 
  __________(total = sum(count),
